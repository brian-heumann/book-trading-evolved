...
def summary_stats(r, riskfree_rate=0.03):
    ...
    ann_sr = r.aggregate(
        sharpe_ratio, riskfree_rate=riskfree_rate, periods_per_year=12)
...
def terminal_stats(rets, floor=0.8, cap=np.inf, name="Stats"):
    ...
    e_short = (floor-terminal_wealth[breach]
               ).mean() if breach.sum() > 0 else np.nan
    e_surplus = (-cap+terminal_wealth[reach]
                 ).mean() if reach.sum() > 0 else np.nan
...
def floor_allocator(psp_r, ghp_r, floor, zc_prices, m=3):
    ...
    for step in range(n_steps):
        ...
        # recompute the new account value at the end of this step
        account_value = psp_alloc * \
            (1+psp_r.iloc[step]) + ghp_alloc*(1+ghp_r.iloc[step])
